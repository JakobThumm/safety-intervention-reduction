...
def sometimes_info(*args, **kwargs):
    ...
    # for this env.
    return dict(cost_exception=0, goal_met=False, speed_agent=0, shield_active=0)
...
class SafetyGymEnvWrapper(Wrapper):
    ...
    def step(self, action):
        ...
        info["speed_agent"] = np.linalg.norm(self.env.world.robot_vel())
...
class SafetyGymTrajInfo(TrajInfo):
    ...
    def __init__(self, **kwargs):
        ...
        self.Cost = 0
        self.SafeCollisions = 0
        self.Collisions = 0
        self.ShieldActivation = 0
        self.ReturnWithoutShield = 0

    def step(self, observation, action, reward, done, agent_info, env_info):
        super().step(observation, action, reward, done, agent_info, env_info)
        cost_all = getattr(env_info, "cost", 0)
        self.Cost += cost_all
        collisions = cost_all - getattr(env_info, "cost_vases_displace", 0) - getattr(env_info, "cost_shield", 0) # vases moving and shield activating are not collisions
        self.Collisions += collisions
        self.SafeCollisions += cost_all - getattr(env_info, "cost_shield", 0) if getattr(env_info, "speed_agent", 0) < 1e-2 else 0
        self.ShieldActivation += getattr(env_info, "shield_active", 0)
        self.ReturnWithoutShield += reward
        if config_shield.REWARD_SHIELD:
            self.ReturnWithoutShield -= getattr(env_info, "shield_active", 0)*config_shield.REWARD_PENALTY # REWARD_PENALTY is negative
