...
class MinibatchRlBase(BaseRunner):
    ...
    def __init__(
            ...
            cost_limit_end_value=None,
            ):
        self.cost_linear_decrease = cost_limit_linear_decrease
        self.cost_limit_end_value = cost_limit_end_value
        n_steps = int(n_steps)
        if cost_limit_linear_decrease and cost_limit_end_value is None:
            raise ValueError('cost_linear_decrease is set to true but cost_limit_end_value is None')
...
class MinibatchRl(MinibatchRlBase):
    ...
    def train(self):
        ...
        if self.cost_linear_decrease:
            cost_limit_end = self.cost_limit_end_value/self.algo.cost_scale
        cost_limit_decrement = (self.algo.cost_limit - cost_limit_end) / n_itr
        for itr in range(n_itr):
            ...
            with logger.prefix(f"itr #{itr} "):
                ...
                self.algo.cost_limit -= cost_limit_decrement
                self.store_diagnostics(itr, traj_infos, opt_info)
                if (itr + 1) % self.log_interval_itrs == 0:
                    self.log_diagnostics(itr)
        self.shutdown()

    def run(self, n_steps, shield):
        config_shield.activate_shield(shield)
        ...
        # for itr in range(n_itr):
        logger.set_iteration(itr)
        with logger.prefix(f"eval"):
            self.agent.eval_mode(itr)  # Might not be this agent sampling.
    ...
    def log_diagnostics(self, itr, prefix='Diagnostics/'):
        with logger.tabular_prefix(prefix):
            ...
            logger.record_tabular('CostLimit', self.algo.cost_limit)
